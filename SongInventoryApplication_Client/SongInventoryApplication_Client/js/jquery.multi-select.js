      ...
      if (haveToSelect && selectedUl.children('li[ms-value="'+value+'"]').length == 0){
        ...
        if (selectableLi.hasClass(ms.data('settings').disabledClass)){
          ...
          selectedLi.click(function(e){
            /*var me=$(this);
            if (e.shiftKey) {
                if (me.prevUntil('.ms-selected-els').length!=me.prevAll().length) {
                    me.prevUntil('.ms-selected-els')
                      .addClass('ms-selected-els')
                      .last().prev()
                      .addClass('ms-selected-els')
                      .prevAll().removeClass('ms-selected-els');
                } else if (me.nextUntil('.ms-selected-els').length!=me.nextAll().length) {
                    me.nextUntil('.ms-selected-els')
                      .addClass('ms-selected-els')
                      .last().next()
                      .addClass('ms-selected-els')
                      .nextAll().removeClass('ms-selected-els');
                }
            } else if (e.ctrlKey) {
            } else me.siblings().removeClass('ms-selected-els');
            me.addClass('ms-selected-els');*/
